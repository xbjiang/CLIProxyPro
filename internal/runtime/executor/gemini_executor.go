...
func (e *GeminiExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		errMsg := helps.SummarizeErrorBody(httpResp.Header.Get("Content-Type"), b)
		helps.LogWithRequestID(ctx).Debugf("request error, error status: %d, error message: %s", httpResp.StatusCode, errMsg)
...
func (e *GeminiExecutor) CountTokens(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	reporter := helps.NewUsageReporter(ctx, e.Identifier(), baseModel, auth)
	reporter.SetStatusCode(resp.StatusCode)
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		errMsg := helps.SummarizeErrorBody(resp.Header.Get("Content-Type"), data)
		helps.LogWithRequestID(ctx).Debugf("request error, error status: %d, error message: %s", resp.StatusCode, errMsg)
		reporter.PublishFailure(ctx, resp.StatusCode, errMsg)
